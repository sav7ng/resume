...
- 最大宽度 `768px` 的中央资料轨道，轨道本身保持透明，让背景网格与贯穿线连成整体
- 区块边界使用「全屏贯穿线」：`100vw` 宽的 1px 细线从视口左缘延伸到右缘，穿过轨道两侧留白
- 区块之间是 `32px` 高的斜纹分隔带，`315°` 斜线配 `10px` 网格，上下各收一条贯穿线
- Hero 顶部（`FIG_001`）是一组**等距积木标志**：真·30° 等距投影的抽象方块城，侧面斜纹填充、
  背景贯穿虚线、鼠标跟随的径向渐变描边，以及点按下沉的 spring 动效。几何由构建脚本按网格算法生成，
  运行时交互为原生 JS（复刻自 chanhdai 的 isometric mark 技法，未沿用其「CD」缩写字形）
- 高分屏（`min-resolution: 2dppx`）下容器与卡片边框自动降为 `0.5px`，贯穿线与积木棱线相应收细
- `Geist` / `Geist Mono` 字体，中文回退到 `PingFang SC` 和系统无衬线字体，正文 `15px / 1.6`
